...
#    Updated: 2022/03/31 00:29:23 by mli              ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
def howManyMedalsByCountry(df: pd.DataFrame, country: str) -> dict:
    ...
    df_teams_wo_dupl = df[df['Sport'].isin(team_sports)].drop_duplicates()
    df_solo = df[~df['Sport'].isin(team_sports)]
    df = pd.concat([df_teams_wo_dupl, df_solo]).loc[:, ['Year', 'Medal']]

    res = df.groupby(['Year'])['Medal'].apply(list).to_dict()
    for year in res.keys():
        res[year] = {
            'G': res[year].count('Gold'),
            'S': res[year].count('Silver'),
            'B': res[year].count('Bronze')
        }
    return (res)
